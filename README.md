...
| 관리자 | 참가자 목록, 관리자 지정/회수, **비밀번호 0000 초기화**, 계좌 초기화(리그 리셋), 참가자 삭제 |
| 앱 설치 | PWA. 홈 화면에 추가하면 주소창 없는 앱처럼 실행된다 |
| AI 진단 | 내 계좌·매매 기록으로 분산·위험관리·매매습관을 S~D 등급으로 진단. **API 키 없이도 동작** (규칙 기반), 키를 넣으면 Claude가 진단 |
| 종목 AI 분석 | 종목 화면의 ✨ AI 분석 버튼. 1년치 일봉으로 추세·RSI·변동성·최대낙폭·52주 위치를 계산해 설명. API 키 불필요 |
| 종목 정렬 | 전체 / 급상승 / 인기많은(리그 친구들이 담은 수) / 거래량상위 |
...
| `ANTHROPIC_API_KEY` | | (없음) | 넣으면 계좌 진단이 Claude로 바뀐다. 비우면 규칙 기반 진단 (무료) |
...
관리자는 참가자 목록을 보고, 계좌를 초기화하고, 참가자를 삭제하고,
**다른 사람에게 관리자 권한을 줄 수 있다.** 인원 제한은 없다.

관리자가 되는 방법은 두 가지다.

1. **관리자 화면에서 지정** — 관리 → 참가자 목록 → *관리자 지정*.
   즉시 반영되며 상대가 다시 로그인할 필요도 없다.
2. **`ADMIN_EMAILS` 환경변수** — 여기 적힌 이메일은 가입할 때, 그리고
   **로그인할 때마다** 자동으로 관리자가 된다. 이미 가입한 사람을
   나중에 추가해도 다음 로그인에 반영된다.

### 안전장치

- 자기 권한은 스스로 내릴 수 없다 (실수로 모두가 잠기는 것을 막는다).
  내리려면 다른 관리자에게 부탁해야 한다.
- 관리자는 최소 한 명 남는다.
- `ADMIN_EMAILS` 에 있는 사람은 권한을 회수해도 **다음 로그인에 다시 관리자가 된다.**
  영구히 내리려면 환경변수에서 먼저 빼고 재배포할 것.

## 비밀번호를 잊어버렸을 때

앱에 이메일 발송 기능이 없어서, **관리자가 초기화**해준다.

1. 관리자: 관리 → 참가자 목록 → **비번 초기화** → 비밀번호가 `0000` 이 된다
2. 학생: `0000` 으로 로그인 → **비밀번호 변경 화면으로 자동 이동**
3. 새 비밀번호(6자 이상, `0000` 불가)로 바꾸면 그때부터 앱을 쓸 수 있다

- 계좌·보유 종목·거래 기록은 **그대로 남는다** (삭제 후 재가입과 다른 점)
- `0000` 은 모두가 아는 번호라, 초기화 상태에서는 비밀번호를 바꾸기 전까지 다른 화면에 **아예 못 들어가게** 막았다.
  초기화했으면 본인에게 바로 알려서 먼저 바꾸게 하자.
- 관리자 자기 비밀번호는 초기화 대신 상단 **비밀번호** 메뉴에서 바꾼다.
- 누구나 상단 **비밀번호** 메뉴에서 언제든 비밀번호를 바꿀 수 있다.

## AI 분석 (두 가지 엔진)

### 1) 규칙 기반 — 기본, API 키 불필요

`ANTHROPIC_API_KEY` 가 없으면 자동으로 이 엔진이 돈다. 외부 호출이 없어 **비용 0원, 횟수 제한 없음, 결과 즉시**.

- **종목 AI 분석** (`/api/ai/stock`): 실제 1년치 일봉 종가로 계산한다.
  20·60일 이동평균 추세, RSI(14), 연 변동성, 1년 고점 대비 위치, 최대 낙폭 → 종목 "온도"와 설명 문장.
- **계좌 AI 진단**: 종목 수, 한 종목 비중, 현금 비중, 큰 손실 방치, 매매 빈도·비용, 국내/해외 편중을 점수화.
  Claude 진단과 **같은 형태**로 결과를 만들어 화면을 그대로 쓴다.

> 참고: 흔히 "API 없이 되는 AI 분석"은 오늘 올랐으면 무조건 "강력 매수", 목표가는 현재가 × 1.22 같은
> **고정 문구**인 경우가 많다. 이 앱은 그렇게 만들지 않았다. 모든 문장은 실제 가격으로 계산한 숫자에서 나오고,
> 매수/매도 권유·목표가·주가 예측은 하지 않는다 (학생용 교육 앱이므로).

### 2) Claude — 선택, `ANTHROPIC_API_KEY` 필요

키를 넣으면 계좌 진단이 Claude(`claude-opus-5`)로 바뀐다. 규칙보다 맥락을 읽고 문장이 자연스럽다.

- 보내는 데이터: 보유 종목·비중·수익률, 최근 체결 30건, 매매 통계, 리그 순위. **이름·이메일은 보내지 않는다.**
- 출력은 JSON 스키마로 강제하고 서버에서 한 번 더 검증한다. `effort: medium`.
- 안전 분류기가 드물게 거절하면 서버 쪽 `fallbacks: "default"` 로 권장 모델이 자동 재시도한다.
- **비용 통제**: 1인당 최근 24시간 3회, 연속 요청 사이 10분. 결과는 DB에 저장해 다시 볼 때는 과금되지 않는다.
  한 번 진단에 약 100~200원, 10명이 매일 3번씩 꽉 채워 써도 한 달 약 9만~18만원이 상한이다.
