...
| AI 진단 | Claude가 내 계좌·매매 기록을 보고 분산·위험관리·매매습관을 등급(S~D)과 함께 진단 |
...
| `ANTHROPIC_API_KEY` | | (없음) | AI 진단용. 비우면 기능이 꺼진다 |
...
`ANTHROPIC_API_KEY` 를 넣으면 **AI 진단** 메뉴가 동작한다.

- 모델: `claude-opus-5`. 요약형 리포트라 `effort: medium` 으로 응답 시간을 줄였다.
- 보내는 데이터: 보유 종목·비중·수익률, 최근 체결 30건, 매매 통계, 리그 순위.
  **이름과 이메일은 보내지 않는다.**
- 출력은 JSON 스키마로 강제하고 서버에서 한 번 더 검증한다.
- 특정 종목 매수/매도 권유와 주가 예측은 하지 않도록 지시했다. 점수는 수익률이 아니라 습관 기준.
- 안전 분류기가 드물게 거절하면 서버 쪽 `fallbacks: "default"` 로 권장 모델이 자동 재시도한다.
- **비용 통제**: 1인당 최근 24시간 3회, 연속 요청 사이 10분. 결과는 DB에 저장해 다시 볼 때는 과금되지 않는다.
  한 번 진단에 약 100~200원 수준이다 (입력 수천 토큰 × $5/M + 출력·추론 수천 토큰 × $25/M).
  10명이 매일 3번씩 꽉 채워 써도 한 달 약 9만~18만원이 상한이다.
