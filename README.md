...
| 기능 | 설명 |
|---|---|
| 회원가입 / 로그인 | **구글 로그인** 또는 이메일+비밀번호. 초대코드로 우리끼리만 가입. 비밀번호는 bcrypt 해시, 세션은 httpOnly JWT 쿠키 |
| 시드머니 | 가입 즉시 1,000만원 지급 (금액 변경 가능) |
| 종목 찾기 | **전체종목 목록**에서 바로 고르기 (코스피/코스닥/ETF/해외 탭). 목록 내 검색과 Yahoo 전체 검색 모두 지원 |
...
| 관리자 | 참가자 목록, 계좌 초기화(리그 리셋), 참가자 삭제 |
| 앱 설치 | PWA. 홈 화면에 추가하면 주소창 없는 앱처럼 실행된다 |
...
| `GOOGLE_CLIENT_ID` | | (없음) | 구글 로그인용. 비우면 구글 버튼이 안 보인다 |
| `GOOGLE_CLIENT_SECRET` | | (없음) | 위와 세트 |
...
`GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` 을 채우면 로그인·가입 화면에 구글 버튼이 생긴다.
비워두면 버튼이 아예 안 보이고 이메일 가입만 동작한다.

1. https://console.cloud.google.com 접속 → 프로젝트 생성
2. **API 및 서비스 → OAuth 동의 화면** → User Type **외부** → 앱 이름·이메일만 채우고 저장
3. **API 및 서비스 → 사용자 인증 정보 → 사용자 인증 정보 만들기 → OAuth 클라이언트 ID**
   - 애플리케이션 유형: **웹 애플리케이션**
   - **승인된 리디렉션 URI** 에 두 개 등록:
     ```
     http://localhost:3000/api/auth/google/callback
     https://<배포주소>/api/auth/google/callback
     ```
4. 발급된 **클라이언트 ID / 보안 비밀번호** 를 환경변수에 넣고 재배포

### 동작 방식

- **기존 이메일 계정과 같은 주소로 구글 로그인** → 그 계정에 구글이 연결되고 로그인된다. 계정이 둘로 갈라지지 않는다.
- **처음 보는 구글 계정** → 초대코드를 통과해야 가입된다. 가입 화면에서 초대코드를 먼저 입력한 뒤 구글 버튼을 눌러야 한다.
- 구글로 가입한 계정은 비밀번호가 없어서 이메일 로그인이 막히고, 안내 문구가 뜬다.
- CSRF 방지를 위해 `state` 에 nonce 를 실어 보내고 쿠키와 대조한다.

## 앱처럼 설치하기

PWA로 만들어져 있어서 스토어 없이 홈 화면에 설치된다.

- **안드로이드 / 크롬** — 접속하면 하단에 설치 배너가 뜬다. 없으면 메뉴(⋮) → *앱 설치*
- **아이폰 / 사파리** — 공유 버튼 → *홈 화면에 추가*
- **데스크톱 크롬 / 엣지** — 주소창 오른쪽 설치 아이콘

설치하면 주소창 없이 전체화면으로 뜨고, 아이콘도 별도로 생긴다.
오프라인일 때는 안내 화면이 나온다. 시세와 잔고는 캐시하지 않으므로
항상 최신 값이다.
